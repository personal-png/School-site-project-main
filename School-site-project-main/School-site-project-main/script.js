...
const gameData = {
    ...
    quests: {
        ...
        '4.2': { completed: false, xp: 20, unlocked: false }
    }
};
...
const levels = [
    ...
    { level: 4, xpNeeded: 90, rank: "Системный администратор" }
];
...
function checkLevelUnlocks() {
    ...
    // Уровень 4 разблокируется при 90 XP
    if (gameData.xp >= 90) {
        unlockQuests(['4.1', '4.2']);
    }
}
...
function showLevelCompletionModalWithAnimation(level, xpGained) {
    ...
    if (level < 3) {
        ...
        // Убираем обработчик для уровня 1 и 2
...
function checkLevelCompletion() {
    ...
    if (isLevelCompleted(currentLevel)) {
        ...
        // Показываем модальное окно завершения уровня с анимацией получения XP
        showLevelCompletionModalWithAnimation(currentLevel, xpGained);
    }
}
...
const consoleState = {
    ...
    expectedCommands: {
        ...
        '3.1': ['mkdir учеба', 'cd учеба', 'mkdir проекты', 'cd проекты', 'mkdir тест', 'cd тест', 'touch задание'],
...
function checkQuest31Completion() {
    // Проверяем существование структуры папок "учеба/проекты/тест" и файла "задание"
    const basePath = '/home/user';
    const expectedStructure = [
        { path: `${basePath}/учеба`, type: 'directory' },
        { path: `${basePath}/учеба/проекты`, type: 'directory' },
        { path: `${basePath}/учеба/проекты/тест`, type: 'directory' },
        { path: `${basePath}/учеба/проекты/тест/задание`, type: 'file' }
    ];
    ...
    // Дополнительная проверка: файл "задание" должен быть именно в папке "тест", а не в других папках
    // Проверяем, что файл не находится в неправильных местах (например, в папке "проекты" вместо "тест")
    const wrongLocations = [
        `${basePath}/учеба/проекты/задание`,
        `${basePath}/учеба/задание`,
        `${basePath}/задание`
    ];
    ...
    for (const wrongPath of wrongLocations) {
        ...
        if (consoleState.files[wrongDirPath] && consoleState.files[wrongDirPath].includes(wrongFileName)) {
            ...
            structureComplete = false;
            missingElements.push(`файл "задание" находится в неправильном месте (${wrongDirPath})`);
            break;
        }
    }
